...
defmodule Noizu.EmailService.V3.Email.Binding do
  ...
  def update_version(%{vsn: 1.0} = entity, _context, _options) do
    ...
    effective_binding = %Noizu.EmailService.V3.Email.Binding.Substitution.Legacy.Effective{
      bind: bind,
      bound: bound,
  ...
  """
  def bind_from_template(%TransactionalEmail{} = txn_email, %Template.Entity{} = template, context, options) do
    #-------------------------------
    ...
    #-------------------------------------
    effective_binding = Template.Entity.effective_binding(template, binding_input, context, options)
  ...
  #----------------------------
  def prepare_binding_inputs(%TransactionalEmail{} = email, %Template.Entity{} = template, context, options) do
  ...
  #
  #----------------------------
  defp path_valid?([_], nil), do: false
  ...
  defp calculate_binding(%Noizu.SmartToken.V3.Token.Entity{} = smart_token, bindings, context, options) do
    smart_token
    |> Noizu.SmartToken.V3.Token.Entity.bind(bindings, options)
    |> Noizu.SmartToken.V3.Token.Repo.create!(Noizu.ElixirCore.CallingContext.system(context))
  end # end calculate_binding/2
